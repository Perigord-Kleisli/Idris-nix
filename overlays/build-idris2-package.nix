{
  ...
  newAttrs =
    builtins.removeAttrs attrs ["idris2Deps" "pname" "version" "ipkgName" "extraBuildInputs"]
    // {
      meta =
        attrs.meta
        // {
...
in
  stdenv.mkDerivation ({
      ...
      installPhase = ''
        ...
        ${
          if isLibrary
          then "${idris2-cmd} --install ${ipkgName}.ipkg ${lib.escapeShellArgs idris2InstallOptions}"
          else ""
        }
        ${
          if hasDocs
          then "IDRIS_DOC_PATH=$out/doc ${idris2-cmd} --installdoc ${ipkgName}.ipkg ${lib.escapeShellArgs idris2DocOptions} || true"
          else ""
        }
